...
class HGModel(BaseModel):
    ...
    def computeSDR(self, prediction_d, targets):
        ...
        for i in range(0, prediction_d.size(0)):
            ...
            z_A_arr = torch.gather(
                torch.index_select(predict_depth, 1, x_A_arr.cuda()),
                0,
                y_A_arr.view(1, -1).cuda(),
            )  # predict_depth:index(2, x_A_arr):gather(1, y_A_arr:view(1, -1))
            z_B_arr = torch.gather(
                torch.index_select(predict_depth, 1, x_B_arr.cuda()),
                0,
                y_B_arr.view(1, -1).cuda(),
            )
    ...
    def evaluate_RMSE(self, input_images, prediction_d, targets):
        ...
        for i in range(0, mask_0.size(0)):
            ...
            total_loss += self.rmse_Loss(
                prediction_d[i, :, :], mask_0[i, :, :], d_gt_0[i, :, :]
            )
            count += 1
