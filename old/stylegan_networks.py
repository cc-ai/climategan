...
"""
    ...
    @notice:
             1) refactor the module of Gsynthesis with
                ...
                - GBlock.
                and etc.
             2) the initialization of every patch we use are all abided by the original
                NvLabs released code.
...
class ApplyStyle(nn.Module):
    """
        @ref: https://github.com/lernapparat/
        lernapparat/blob/master/style_gan/pytorch_style_gan.ipynb
    """
...
class FC(nn.Module):
    def __init__(
        ...
        """
            ...
            Dense/FC/Linear Layer of original Tensorflow version.
        """
...
class Blur2d(nn.Module):
    def __init__(self, f=[1, 2, 1], normalize=True, flip=False, stride=1):
        ...
        assert (
            isinstance(f, list) or f is None
        ), "kernel f must be an instance of python built_in type list!"
...
class Conv2d(nn.Module):
    ...
    def forward(self, x):
        if self.bias is not None:
            ...
            return F.conv2d(
                x, self.weight * self.w_lrmul, padding=self.kernel_size // 2
            )
...
class Upscale2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.factor > 1:
            ...
            x = x.contiguous().view(
                shape[0], shape[1], self.factor * shape[2], self.factor * shape[3]
            )
        return x
...
class PixelNorm(nn.Module):
    def __init__(self, epsilon=1e-8):
        """
            ...
            gradient-computation-has-been-modified-by-an-inplace-operation/836/3
        """
...
class InstanceNorm(nn.Module):
    def __init__(self, epsilon=1e-8):
        """
            @notice: avoid in-place ops.
            https://discuss.pytorch.org/t/encounter-the-runtimeerror-one-of-the-
            variables-needed-for-gradient-computation-has-been-modified-by-an-inplace-operation/836/3
        """
...
class GBlock(nn.Module):
    def __init__(
        ...
        if res < 7:
            ...
            self.up_sample = nn.ConvTranspose2d(
                self.nf(res - 3), self.nf(res - 2), 4, stride=2, padding=1
            )
        ...
        self.conv1 = Conv2d(
            input_channels=self.nf(res - 2),
            output_channels=self.nf(res - 2),
            kernel_size=3,
    ...
    def forward(self, x, dlatent):
        x = self.up_sample(x)
        x = self.adaIn1(
            x, self.noise_input[self.res * 2 - 4], dlatent[:, self.res * 2 - 4]
        )
        x = self.conv1(x)
        x = self.adaIn2(
            x, self.noise_input[self.res * 2 - 3], dlatent[:, self.res * 2 - 3]
        )
        return x
...
class G_synthesis(nn.Module):
    def __init__(
        ...
        self.channel_shrinkage = Conv2d(
            input_channels=self.nf(self.resolution_log2 - 2),
    ...
    def forward(self, dlatent):
        """
           dlatent: Disentangled latents (W), 
           shape为[minibatch, num_layers, dlatent_size].
        :param dlatent:
        :return:
        """
        images_out = None
        # Fixed structure: simple and efficient, but does not 
        # support progressive growing.
...
class StyleDiscriminator(nn.Module):
    def __init__(
        ...
        self.fromrgb = nn.Conv2d(
            num_channels, self.nf(self.resolution_log2 - 1), kernel_size=1
        )
        self.structure = structure
        ...
        self.down23 = nn.Conv2d(
            self.nf(self.resolution_log2 - 7),
            self.nf(self.resolution_log2 - 7),
            kernel_size=2,
            stride=2,
        )
        self.down24 = nn.Conv2d(
        ...
        self.conv2 = nn.Conv2d(
            self.nf(self.resolution_log2 - 1),
            self.nf(self.resolution_log2 - 2),
            kernel_size=3,
            padding=(1, 1),
        )
        self.conv3 = nn.Conv2d(
            self.nf(self.resolution_log2 - 2),
            self.nf(self.resolution_log2 - 3),
            kernel_size=3,
            padding=(1, 1),
        )
        self.conv4 = nn.Conv2d(
            self.nf(self.resolution_log2 - 3),
            self.nf(self.resolution_log2 - 4),
            kernel_size=3,
            padding=(1, 1),
        )
        ...
        self.conv_last = nn.Conv2d(
            self.nf(self.resolution_log2 - 8), self.nf(1), kernel_size=3, padding=(1, 1)
        )
