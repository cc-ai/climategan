...
# Model features
model_feats = ['masker', 'seg', 'depth', 'dada_seg', 'dada_masker', 'spade', 'pseudo',
        'ground', 'instagan']
...
def add_ci_mean(ax, sample_measure, bs_mean, bs_std, ci, color, alpha, fontsize, invert=False):
    ...
    ax.vlines(x=bs_mean, ymin=0., ymax=dist_y[x_idx_mean], color='k', linewidth=linewidth)
    
    # Add annotation of bootstrap mean
    bbox_props = dict(boxstyle='round, pad=0.4', fc='w', ec='k', lw=2)
    
    ax.text(bs_mean, 0.6 * dist_y[x_idx_mean], s='Bootstrap mean = {:.4f}'.format(bs_mean), 
            ha='center', va='center', fontsize=fontsize,
            bbox=bbox_props)
    
    # Add vertical line of sample_measure
    x_idx_smeas = np.argmin(np.abs(dist_x - sample_measure))
    ax.vlines(x=sample_measure, ymin=0., ymax=dist_y[x_idx_smeas], color='k', linewidth=linewidth, linestyles='dotted')
    
    # Add SD
    bbox_props = dict(boxstyle='darrow, pad=0.4', fc='w', ec='k', lw=2)
    
    ax.text(bs_mean, 0.4 * dist_y[x_idx_mean], s='SD = {:.4f} = SE'.format(bs_std), 
            ha='center', va='center', fontsize=fontsize,
            bbox=bbox_props)
...
def plot_bootstrap_distr(sample_measure, bs_samples, alpha, color_ci, color_pval=None, null=None):
    ...
    # Annotations
    add_ci_mean(ax, sample_measure, bs_mean, bs_std, ci, color=color_ci, alpha=alpha_plot, fontsize=fontsize)
...
if __name__ == "__main__":
    ...
    for mi in df.loc[df[technique]].model_feats.unique():
        for mj in df.model_feats.unique():
            ...
            for f in model_feats:
                if f == technique:
                    continue
                elif df.loc[df.model_feats == mj, f].unique()[0] != df.loc[df.model_feats == mi, f].unique()[0]:
                    is_pair = False
                    break
                else:
                    pass
    ...
    for m_with, m_without in model_pairs:
        ...
        for metric in data.keys():
            diff = df_with.sort_values(by='img_idx')[metric].values - df_without.sort_values(by='img_idx')[metric].values
...
for metric in dict_metrics['key_metrics']:
    ...
    fig, bs_mean, bs_std, ci, pval = plot_bootstrap_distr(sample_measure, bs_data['20_trimmed_mean'][metric], alpha=args.alpha, color_ci=color_cat1_light, color_pval=color_cat2_light, null=0.)

    # Save figure
    output_fig = output_dir / '{}_bootstrap_{}_{}.png'.format(args.technique, metric, '20_trimmed_mean')
    fig.savefig(output_fig, dpi=fig.dpi, bbox_inches='tight')

    # Store results
    output_results = output_dir / '{}_bootstrap_{}_{}.yml'.format(args.technique, metric, '20_trimmed_mean')
    results_dict = {
